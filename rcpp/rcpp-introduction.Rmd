...
I've written a Ricker/logistic population dynamic simulation in `rcpp/ricker.R`. Take that code and translate it into C++ for use with Rcpp. Name your new function `ricker_rcpp()`. I've started for you in `rcpp/ricker.cpp`.

Verify that you get the same answer out of both functions if you start with the same random number generator seed (e.g., `set.seed(42)`).
...
sourceCpp(here::here("rcpp/ricker.cpp"))
...
Take your file `rcpp/ricker.cpp` and copy its contents into `rcpp/ricker-df.cpp`. Edit the end of your function so that it returns a data frame instead of a numeric vector. Make your data frame include the columns year (`Rcpp::seq(1, n)`), `x` (for the simulated population density), and a column for each parameter.
