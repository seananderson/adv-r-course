...
tic()
p_future_lapply <- profvis::profvis({
  y <- future.apply::future_lapply(x, quantile, probs = 1:3/4)
})
toc()
p_future_lapply
```

We can pass global variables and packages if future has trouble figuring these out automatically via the `future.packages` and `future.globals` arguments to `future.apply::future_lapply()`.

# Exercise

TODO

basic exercise with `future.apply::future_lapply` as a replacement for a loop.
