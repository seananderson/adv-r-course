...
The plyr package popularized the "split-apply-combine" approach, but is no longer actively developed and it's time to move on. Unfortunately, there isn't a single clear best way to work at the moment in the last couple of years has seen a rapid evolution of suggested approaches within the tidyverse.
...
Note that some of these return the grouping column in the output. Conversely, some of them make the grouping variable available within the `get_slope()` function others do not. Some require an explicit `dplyr::ungroup()` and others do not.
