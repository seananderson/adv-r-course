...
```{r}
d <- readRDS("data/pcod-syn-hs.rds")
glimpse(d)
...
A more rigourous test would be with benchmarking. You've worked with this in an earlier lesson, so try filling within yourself. I recommend using a smaller number of bootstrap repetitions, say 24.

```{r}
b <- bench::mark(
  sequential = boot_biomass(d, reps = 24), # exercise
  future.apply = boot_biomass_future.apply(d, reps = 24), # exercise
  purrr_futur = boot_biomass_purrr_future(d, reps = 24), # exercise
  furrr = boot_biomass_furrr(d, reps = 24), # exercise
...
Try extending our above functions (e.g. `boot_biomass()`) so that it also works with multiple surveys. Then pick one of the above approaches to iterate over years and surveys in parallel.

Here is the data for Hecate Strait, Queen Charlotte Sound, and West Coast Vancouver Island:

```{r}
d_multiple <- readRDS("data/pcod-syn-hs-qcs-wcvi.rds")
glimpse(d_multiple)
...
Write versions of the above functions to iterate over years and surveys in parallel. Save your output to an object named `b`. Check to make sure your parallel version is faster.
...
tic() # exercise
b <- boot_biomass2(d_multiple, reps = 200) # exercise
toc() # exercise
b # exercise

tic() # exercise
b <- boot_biomass_future.apply2(d_multiple, reps = 200) # exercise
toc() # exercise
b # exercise
```
